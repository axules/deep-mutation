...
describe('mutate with deep', () => {
  test.each(data)('mutateDeep(*, %s)', (title, obj, ext, expected) => {
    expect(mutateDeep(obj, ext)).toEqual(expected);
  });

  test('mutateDeep is the same that mutate.deep', () => {
    expect(mutateDeep).toBe(mutate.deep);
  });
});